<?php

/* @var $this \yii\web\View */
/* @var $content string */

use app\widgets\Alert;
use yii\helpers\Html;
use yii\bootstrap\Nav;
use yii\bootstrap\NavBar;
use yii\widgets\Breadcrumbs;
use app\assets\AppAsset;

AppAsset::register($this);
?>


<?php $this->beginPage() ?>

<!DOCTYPE html>
<html lang="<?= Yii::$app->language ?>">

<head>
    <meta charset="<?= Yii::$app->charset ?>">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <?= Html::csrfMetaTags() ?>
    <title><?= Html::encode($this->title) ?></title>
    <?php $this->head() ?>
</head>

<body>


<?php $this->beginBody() ?>

<div class="wrap">
    <div style="position: fixed;">
    <?php
            NavBar::begin([
                'brandLabel' => Yii::$app->name,
                'brandUrl' => Yii::$app->homeUrl,
                'options' => [
                    'class' => 'navbar navbar-inverse',
                    //'style' => 'background-color: #cdc7bb; box-shadow: 0 0 1pc;'
                    'style' => 'background-color: #161a1e; box-shadow: 0 0 1pc;'
                ],
            ]);
            echo Nav::widget([
                'options' => [
                        'class' => 'nav navbar-nav navbar-inverse navbar-right',
                        'style' => 'background-color: #161a1e;'
                    ],
                'items' => [
                    ['label' => 'Develop' ,
                        'items' => [
                                ['label' => 'control', 'url' => ['/control']],
                                ['label' => 'Users', 'url' => ['/control/users']],
                                ['label' => 'Test', 'url' => ['/test']],
                                ['label' => 'Articles', 'url' => ['/control/articles']]
                        ]
                    ],
                    ['label' => 'Test', 'url' => ['/site/test']],
                    ['label' => 'Обратная связь', 'url' => ['/site/contact']],
                    Yii::$app->user->isGuest ? (
                            ['label' => 'Options',
                                'items' => [
                                        ['label' => 'Enter', 'url' => ['/site/login']]]
                            ]
                    ) : (
                            '<li>'
                            . Html::beginForm(['/site/logout'], 'post')
                            //. Html::dropDownList()
                            . Html::submitButton(
                                    'Logout (' . Yii::$app->user->identity->username . ')',
                                    ['class' => 'btn btn-link logout']
                            )
                            . Html::endForm()
                            . '</li>'
                    ),
                ],
            ]);
            NavBar::end();
            ?>
        </div>
    <br>
    <br>
    <br>

    <div style="background-color: #fff;" class="container">
        <br>
        <?php echo Breadcrumbs::widget([
            'links' => isset($this->params['breadcrumbs']) ? $this->params['breadcrumbs'] : [],]) ?>


        <?= Alert::widget() ?>

        <?= $content ?>

    </div>
</div>

<footer class="footer">
    <div class="container">
        <p class="pull-left">&copy; My Company <?= date('Y') ?></p>

        <p class="pull-right"><?= Yii::powered() ?></p>
    </div>
</footer>

<?php yii\helpers\Url::remember(); ?>

<?php $this->endBody() ?>

</body>
</html>

<?php $this->endPage() ?>
